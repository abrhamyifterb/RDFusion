...
import {
  FileTreeProvider,
  watchFiles,
} from "./presentation/activity-bar/file-tree-provider";
import { WorkspaceIndexProgress } from "./utils/workspace-notifier";
import { WorkspaceIndexService } from "./utils/workspace-index-service";
import {
...
import {
  defaultTurtleFormatConfig,
  turtleFormattingLabels,
} from "./default-config/turtle-formatting-config";
import {
  defaultIriSchemeConfig,
  IriSchemeConfigLabels,
} from "./default-config/Iri-scheme-config";
...
export async function activate(context: ExtensionContext) {
    ...
    initializationOptions: {
      rdfusion: {
        ...
        performance: {
          trace: workspace
            .getConfiguration("rdfusion")
            .get("performance.trace", false),
        },
      },
    },
  };
  const trace = vscode.window.createOutputChannel("RDFusion LSP Trace");
  const rdfusionOutput = vscode.window.createOutputChannel("RDFusion");
  context.subscriptions.push(trace, rdfusionOutput);

  client = new LanguageClient(
    "languageServerExample",
    "RDFusion Language Server",
    serverOptions,
    {
      ...clientOptions,
  ...
  const workspaceGlob =
    config.get<string>("vocabularyGlob") || "**/*.{ttl,jsonld}";

  const workspaceIndexService = new WorkspaceIndexService(client, {
    globPattern: workspaceGlob,
    output: rdfusionOutput,
    watchDebounceMs: 750,
  });
  context.subscriptions.push(workspaceIndexService);
  workspaceIndexService.startWatching(context);

  const indexShaclWorkspace = async (
    onProgress?: (progress: WorkspaceIndexProgress) => void,
  ) => workspaceIndexService.indexWorkspaceShacl(onProgress);

  const indexRdfWorkspace = async (
    onProgress?: (progress: WorkspaceIndexProgress) => void,
  ) => workspaceIndexService.indexWorkspaceRdf(onProgress);

  context.subscriptions.push(
    vscode.commands.registerCommand("rdfusion.openShaclSelection", () => {
      ShaclSelectionPanel.createOrShow(
    ...
    vscode.commands.registerCommand("rdfusion.openShaclCoverage", () => {
      ShaclCoveragePanel.createOrShow(
        context.extensionUri,
        client,
        indexRdfWorkspace,
        rdfusionOutput,
  ...
  function refreshStatus() {
    ...
    const tValid = config.get<Record<string, boolean>>(
      "turtle.validations",
      defaultTurtleValidations,
    );
    const jValid = config.get<Record<string, boolean>>(
      "jsonld.validations",
      defaultJsonLdValidations,
    );
    const tAuto = config.get<Record<string, boolean>>(
      "turtle.autocomplete",
      defaultTurtleAutocomplete,
    );
    const jAuto = config.get<Record<string, boolean>>(
      "jsonld.autocomplete",
      defaultJsonLdAutocomplete,
    );
    const tFormat = config.get<Record<string, boolean | number>>(
      "turtle.formatting",
      defaultTurtleFormatConfig,
    );
    const iriValid = config.get<Record<string, boolean | string>>(
      "common.validations",
      defaultIriSchemeConfig,
    );
    ...
    statusBar.text = `$(checklist) RDFusion ${enabledCount}/${totalCount}`;
    statusBar.tooltip =
      "RDFusion settings enabled. Run an RDFusion configuration command to change them.";
    statusBar.show();
  }
  ...
  async function configureSection<K extends string>(
    ...
    const config = workspace.getConfiguration("rdfusion");
    const current = config.get<Record<string, boolean | number | string>>(
      configKey,
      defaults,
    );
    ...
    const picked = await vscode.window.showQuickPick(items, {
      canPickMany: true,
      placeHolder: `Select ${sectionName} options to enable`,
    });
    ...
    for (const k of Object.keys(defaults) as K[]) {
      ...
      if (typeof defaultValue === "boolean") {
        ...
        if (isSelected) {
          const input = await vscode.window.showInputBox({
            prompt: `Enter values for ${labelMap[k]} (comma-separated)`,
            value: current[k]?.toString() ?? defaultValue.toString(),
          });
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("rdfusion.configureTurtleValidations", () =>
      configureSection(
        "turtle.validations",
        defaultTurtleValidations,
        turtleValidationLabels,
    ...
    vscode.commands.registerCommand("rdfusion.configureJsonldValidations", () =>
      configureSection(
        "jsonld.validations",
        defaultJsonLdValidations,
        jsonLdValidationLabels,
    ...
    vscode.commands.registerCommand(
      ...
      () =>
        configureSection(
          ...
          defaultJsonLdAutocomplete,
          jsonLdAutocompleteLabels,
          "JSON-LD Autocomplete",
        ),
    ),
    vscode.commands.registerCommand("rdfusion.configureTurtleFormatting", () =>
      configureSection(
        "turtle.formatting",
        defaultTurtleFormatConfig,
        turtleFormattingLabels,
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("rdfusion.formatTriples", async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        return vscode.window.showErrorMessage("Open an RDF document first.");
      }
      ...
      try {
        ...
        vscode.window.showErrorMessage(`RDFusion command failed: ${err instanceof Error ? err.message : String(err)}`);
      }
    }),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("rdfusion.generateVoID", async () => {
      ...
      const uri = editor.document.uri.toString();
      const generatedVoID: string = await client.sendRequest(
    ...
    vscode.commands.registerCommand("rdfusion.filterTriples", async () => {
      ...
      const ask = (label: string) =>
        vscode.window.showInputBox({
          prompt: label,
          placeHolder: 'e.g. ex:Bart, foaf:name, "Simpson"',
        });
    ...
    vscode.commands.registerCommand(
      ...
      async () => {
        ...
        const ask = (label: string) =>
          vscode.window.showInputBox({
            prompt: label,
            placeHolder: "e.g. ex:Bart, <http://example.org/lisa>, ex:Person",
          });
      ...
      async () => {
        ...
        const ask = (label: string) =>
          vscode.window.showInputBox({
            prompt: label,
            placeHolder:
              "e.g. foaf:mbox, ex:knows, <http://xmlns.com/foaf/0.1/name>",
          });

        const pred = await ask(
          "Enter one or more predicates (comma-separated):",
        );
    ...
    vscode.commands.registerCommand(
      ...
      async () => {
        ...
        const ask = (label: string) =>
          vscode.window.showInputBox({
            prompt: label,
            placeHolder: 'e.g. 20, "Simpson", <mailto:bart@example.com>',
          });
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("rdfusion.compactJsonld", async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor || editor.document.languageId !== "jsonld") {
        vscode.window.showErrorMessage("Open a JSON-LD document first.");
        return;
      }
      const uri = editor.document.uri.toString();
      const mode = "compact";
  ...
  const setIriMaxLength = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      try {
        ...
        return vscode.window.showErrorMessage(
          `Could not update RDFusion settings: ${e.message}`,
        );
      }
      ...
      vscode.window.showInformationMessage(
        `IRI shortening length set to ${newValue}.`,
      );
    },
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("rdfusion.frameJsonld", async () => {
      ...
      try {
        ...
        vscode.window.showErrorMessage(`JSON-LD framing failed: ${e.message}`);
      }
    }),
  );
    ...
    vscode.commands.registerCommand(
      ...
      async (
        ...
        for (const fileUri of filesToMerge) {
          try {
            ...
            vscode.window.showErrorMessage(
              `Could not read ${fileUri.fsPath}: ${(err as any).message}`,
            );
            return;
          }
        }
        ...
        try {
          ...
          vscode.window.showErrorMessage(
            `Could not merge RDF files: ${(err as any).message}`,
          );
          return;
        }
  ...
  client.onNotification("window/logMessage", (params) => {
    rdfusionOutput.appendLine(String(params.message));
  });
}
