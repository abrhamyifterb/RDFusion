...
function parseStr(text: string, n?: Node): string | null {
  ...
  try {
    return JSON.parse(textSlice(text, n));
  } catch {
    return null;
  }
}
...
export class IriExpectationIndex {
  ...
  private walkWithActive(
    n: Node,
    inheritedCoercions: Set<string>,
    inheritedTerms: Set<string>,
  ) {
    if (n.type === "object") {
    ...
    if (n.type === "array") {
      for (const ch of n.children ?? []) {
        this.walkWithActive(ch, inheritedCoercions, inheritedTerms);
      }
      return;
    }
  }

  private applyLocalContext(
    obj: Node,
    activeCoercions: Set<string>,
    activeTerms: Set<string>,
  ) {
    const ctxProps = (obj.children ?? []).filter(
      (p) =>
        p.type === "property" &&
        p.children?.length === 2 &&
      ...
      if (v.type === "null") {
        ...
        for (const item of v.children ?? []) {
          if (item.type === "null") {
            ...
            this.applyCtxObject(item, activeCoercions, activeTerms);
          }
        }
      } else {
        // 
  ...
  private applyCtxObject(
    ...
    for (const prop of ctxObj.children ?? []) {
      ...
      if (defNode.type === "object") {
        ...
        if (coerces) activeCoercions.add(term);
        else activeCoercions.delete(term);
      } else {
        activeCoercions.delete(term);
      }
    }
